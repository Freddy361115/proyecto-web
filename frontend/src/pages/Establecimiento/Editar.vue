        ...
        <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
            <md-card>
                <md-card-header data-background-color="green">
                    <h4 class="title">Editar Establecimiento</h4>
                </md-card-header>
                <md-card-content>
                    <div class="md-layout">
                        ...
                        <div class="md-layout-item md-small-size-100 md-size-33">
                            ...
                            <md-field>
                                <v-select :loading="loading" :options="supervisores" style="width:100%" type="text" name="supervisor" class="rounded" v-model="supervisor_selected">
                                    <template #no-options>
...
export default {
    ...
    async mounted() {
        ...
        this.establecimiento = await this.$getRequest(process.env.VUE_APP_API + '/establecimientos/' + this.$route.params.id);

        if (this.establecimiento.id_establecimiento) {
            this.supervisor_selected = this.supervisores.find(x => x.code == this.establecimiento.id_supervisor);
        }
    ...
    methods: {
        ...
        async save() {
            ...
            let response = await this.$putRequest(process.env.VUE_APP_API + "/establecimientos/" + this.establecimiento.id_establecimiento, this.establecimiento);
            if (response.success) {
                this.$showNotification('success', 'Establecimiento actualizado correctamente', 'check');
                this.$router.push({
                    name: "Establecimientos"
                });
            }
