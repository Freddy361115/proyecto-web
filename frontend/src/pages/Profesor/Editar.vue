    ...
    <div class="md-layout">
        <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
            <md-card>
                ...
                <md-card-content>
                    <div class="md-layout" v-if="profesor">
...
export default {
    ...
    async mounted() {
        ...
        this.profesor = await this.$getRequest(process.env.VUE_APP_API + '/profesores/' + this.$route.params.id);

        if (this.profesor.id_establecimiento) {
            this.establecimiento_selected = this.establecimientos.find(x => x.code == this.profesor.id_establecimiento);
        }
    ...
    methods: {
        ...
        async save() {
            ...
            let response = await this.$putRequest(process.env.VUE_APP_API + "/profesores/" + this.profesor.id, this.profesor);
            if (response.success) {
                if (!that.file) {
                    //   this.$createdSuccess();
