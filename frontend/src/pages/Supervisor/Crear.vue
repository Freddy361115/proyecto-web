...
export default {
    ...
    methods: {
        ...
        async save() {
            let that = this;
            let response = await this.$postRequest(process.env.VUE_APP_API + "/supervisors", this.supervisor);
            if (response.success) {
                this.$showNotification('success', 'Supervisor creado correctamente', 'check');
                this.$router.push({
                    name: "Supervisores"
                });
            }
